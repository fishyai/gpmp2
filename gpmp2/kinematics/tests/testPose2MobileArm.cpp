...
/* ************************************************************************** */
// fk wrapper
gtsam::Pose3 fkpose(const Pose2MobileArm& r, const Pose2Vector& p, const gtsam::Vector& v, size_t i) {
...
gtsam::Vector3 fkvelocity(const Pose2MobileArm& r, const Pose2Vector& p, const gtsam::Vector& v, size_t i) {
  vector<gtsam::Pose3> pos;
  vector<gtsam::Vector3, Eigen::aligned_allocator<gtsam::Vector3>> vel;
...
TEST(Pose2MobileArm, 2linkPlanarExamples) {
  ...
  q = Pose2Vector(gtsam::Pose2(), gtsam::Vector2(0.0, 0.0));
  qdot = (gtsam::Vector5() << 0, 0, 0, 0, 0).finished();
  qdymc = qdot;
  pvec_exp.clear();
...
/*
  vJp_exp.clear();
  vJp_exp.push_back(numericalDerivativeDynamic(boost::function<gtsam::Vector3(const Pose2Vector&)>(
  ...
  vJv_exp.clear();
  vJv_exp.push_back(numericalDerivativeDynamic(boost::function<gtsam::Vector3(const Vector5&)>(
...
*/
  ...
  q = Pose2Vector(gtsam::Pose2(), gtsam::Vector2(0.0, 0.0));
  qdot = (gtsam::Vector5() << 1, 0, 1, 0, 0).finished();
  qdymc = qdot;
  pvec_exp.clear();
  pvec_exp.push_back(gtsam::Pose3());
  pvec_exp.push_back(gtsam::Pose3(gtsam::Rot3::Ypr(M_PI/4.0, 0, 0), gtsam::Point3(1.707106781186548, 0.707106781186548, 2.0)));
...
/*
  ...
  EXPECT(gtsam::assert_equal(vvec_exp[2], vvec_act[2], 1e-9));
*/
  EXPECT(gtsam::assert_equal(pJp_exp[0], pJp_act[0], 1e-6));
  EXPECT(gtsam::assert_equal(pJp_exp[1], pJp_act[1], 1e-6));
  EXPECT(gtsam::assert_equal(pJp_exp[2], pJp_act[2], 1e-6));
...
*/
  ...
  q = Pose2Vector(gtsam::Pose2(), gtsam::Vector2(0.0, 0.0));
  qdot = (gtsam::Vector5() << 0, 0, 0, 1, 0).finished();
  qdymc = qdot;
  pvec_exp.clear();
  pvec_exp.push_back(gtsam::Pose3());
  pvec_exp.push_back(gtsam::Pose3(gtsam::Rot3::Ypr(M_PI/4.0, 0, 0), gtsam::Point3(1.707106781186548, 0.707106781186548, 2.0)));
...
/*
  ...
  vJp_exp.push_back(numericalDerivativeDynamic(boost::function<gtsam::Vector3(const Pose2Vector&)>(
      boost::bind(&fkvelocity, marm, _1, qdot, size_t(2))), q, 1e-6));
  vJv_exp.clear();
  vJv_exp.push_back(numericalDerivativeDynamic(boost::function<gtsam::Vector3(const gtsam::Vector5&)>(
      boost::bind(&fkvelocity, marm, q, _1, size_t(0))), qdot, 1e-6));
  vJv_exp.push_back(numericalDerivativeDynamic(boost::function<gtsam::Vector3(const gtsam::Vector5&)>(
      boost::bind(&fkvelocity, marm, q, _1, size_t(1))), qdot, 1e-6));
  vJv_exp.push_back(numericalDerivativeDynamic(boost::function<gtsam::Vector3(const gtsam::Vector5&)>(
...
*/
  ...
  q = Pose2Vector(gtsam::Pose2(2.0, -1.0, M_PI/2.0), gtsam::Vector2(M_PI/4.0, 0.0));
  qdot = (gtsam::Vector5() << 0, 0, 0, 0, 0).finished();
  qdymc = qdot;
  pvec_exp.clear();
  pvec_exp.push_back(gtsam::Pose3(gtsam::Rot3::Ypr(M_PI/2.0, 0, 0), gtsam::Point3(2.0, -1.0, 0)));
  pvec_exp.push_back(gtsam::Pose3(gtsam::Rot3::Ypr(M_PI, 0, 0), gtsam::Point3(1.0, 0.0, 2.0)));
  pvec_exp.push_back(gtsam::Pose3(gtsam::Rot3::Ypr(M_PI, 0, 0), gtsam::Point3(0.0, 0.0, 2.0)));
  vvec_exp.clear();
  vvec_exp.push_back(gtsam::Vector3(0, 0, 0));
  vvec_exp.push_back(gtsam::Vector3(0, 0, 0));
  vvec_exp.push_back(gtsam::Vector3(0, 0, 0));
...
*/
  ...
  q = Pose2Vector(gtsam::Pose2(2.3, -1.2, 0.1), gtsam::Vector2(1.2, -0.9));
  qdot = (gtsam::Vector5() << 3.2, 4.9, -3.3, -2.1, 0.1).finished();
  qdymc = qdot;
