...
/* ************************************************************************** */
// fk wrapper
gtsam::Pose3 fkpose(const Pose2Mobile2Arms& r, const Pose2Vector& p, size_t i) {
...
TEST(Pose2Mobile2Arms, 2linkPlanarExamples) {
  ...
  q = Pose2Vector(gtsam::Pose2(), gtsam::Vector4(0.0, 0.0, 0.0, 0.0));
  pvec_exp.clear();
  ...
  q = Pose2Vector(gtsam::Pose2(1.0, 0, M_PI/4.0), gtsam::Vector4(M_PI/4.0, M_PI/4.0, M_PI/4.0, M_PI/4.0));
  pvec_exp.clear();
  pvec_exp.push_back(gtsam::Pose3(gtsam::Rot3::Ypr(M_PI/4.0, 0, 0), gtsam::Point3(1.0, 0, 0)));
  pvec_exp.push_back(gtsam::Pose3(gtsam::Rot3::Ypr(3.0 * M_PI/4.0, 0, 0), gtsam::Point3(1.0, 1.414213562373095, 1.0)));
  pvec_exp.push_back(gtsam::Pose3(gtsam::Rot3::Ypr(4.0 * M_PI/4.0, 0, 0), gtsam::Point3(0.0, 1.414213562373095, 1.0)));
  pvec_exp.push_back(gtsam::Pose3(gtsam::Rot3::Ypr(5.0 * M_PI/4.0, 0, 0), gtsam::Point3(1.0, 0.0, 3.0)));
  pvec_exp.push_back(gtsam::Pose3(gtsam::Rot3::Ypr(6.0 * M_PI/4.0, 0, 0), gtsam::Point3(1.0, -1.0, 3.0)));
  pJp_exp.clear();
  pJp_exp.push_back(numericalDerivativeDynamic(boost::function<gtsam::Pose3(const Pose2Vector&)>(
      boost::bind(&fkpose, marm, _1, size_t(0))), q, 1e-6));
  pJp_exp.push_back(numericalDerivativeDynamic(boost::function<gtsam::Pose3(const Pose2Vector&)>(
  ...
  q = Pose2Vector(gtsam::Pose2(7.5, -2.8, 0.1), gtsam::Vector4(-3.4, 5.3, 4.5, -9.9));
  pJp_exp.clear();
  pJp_exp.push_back(numericalDerivativeDynamic(boost::function<gtsam::Pose3(const Pose2Vector&)>(
      boost::bind(&fkpose, marm, _1, size_t(0))), q, 1e-6));
  pJp_exp.push_back(numericalDerivativeDynamic(boost::function<gtsam::Pose3(const Pose2Vector&)>(
