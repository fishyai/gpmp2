...
double sdf_wrapper(const SignedDistanceField& field, const gtsam::Point3& p) {
...
TEST(SDFutils, test) {
  ...
  idx = field.convertPoint3toCell(gtsam::Point3(0, 0, 0));
  ...
  idx = field.convertPoint3toCell(gtsam::Point3(0.18, -0.18, 0.07));   // tri-linear interpolation
  ...
  EXPECT(gtsam::assert_equal(gtsam::Point3(0.0, -0.1, 0.2), field.convertCelltoPoint3(idx)));

  // gradient
  gtsam::Vector3 grad_act, grad_exp;
  gtsam::Point3 p;
  p = gtsam::Point3(-0.13, -0.14, 0.06);
  field.getSignedDistance(p, grad_act);
  grad_exp = numericalDerivative11(boost::function<double(const gtsam::Point3&)>(
      boost::bind(sdf_wrapper, field, _1)), p, 1e-6);
  EXPECT(gtsam::assert_equal(grad_exp, grad_act, 1e-6));

  p = gtsam::Point3(0.18, 0.12, 0.01);
  field.getSignedDistance(p, grad_act);
  grad_exp = numericalDerivative11(boost::function<double(const gtsam::Point3&)>(
      boost::bind(sdf_wrapper, field, _1)), p, 1e-6);
  EXPECT(gtsam::assert_equal(grad_exp, grad_act, 1e-6));
