...
void Pose2MobileVetLin2Arms::forwardKinematics(
  ...
  gtsam::Pose3 veh_base, tso_base, arm1_base, arm2_base;
  gtsam::Matrix63 Hveh_base;
  gtsam::Matrix64 Htso_base, Harm1_base, Harm2_base;
  ...
  arm1_.forwardKinematics(p.configuration().segment(1, arm1_.dof()), boost::none, arm1jpx, 
      boost::none, J_px_p ? boost::optional<vector<gtsam::Matrix>&>(Jarm1_jpx_jp) : boost::none);
  arm2_.updateBasePose(arm2_base);
  arm2_.forwardKinematics(p.configuration().segment(1+arm1_.dof(), arm2_.dof()), boost::none, arm2jpx, 
      boost::none, J_px_p ? boost::optional<vector<gtsam::Matrix>&>(Jarm2_jpx_jp) : boost::none);
