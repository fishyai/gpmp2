...
Arm::Arm(size_t dof, const gtsam::Vector& a, const gtsam::Vector& alpha, const gtsam::Vector& d,
    const gtsam::Pose3& base_pose, boost::optional<const gtsam::Vector&> theta_bias) :
  ...
  if (theta_bias)
    ...
    theta_bias_ = gtsam::Vector::Zero(dof);
...
void Arm::forwardKinematics(
  ...
  for (size_t i = 0; i < dof(); i++) {
    ...
    jpx[i] = gtsam::Pose3(gtsam::Rot3(Ho[i+1].block<3,3>(0,0)), gtsam::Point3(Ho[i+1].col(3).head<3>()));
